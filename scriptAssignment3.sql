...
SELECT t.title_id AS 'Title Id', t.title_name AS 'Title Name',t.subject_id AS 'Subject Id',
    t.publisher_id AS 'Publisher Id',b.accession_number AS 'Book Id' FROM titles t 
    INNER JOIN books b ON t.title_id = b.title_id
    INNER JOIN book_issue bi ON b.accession_no=bi.accession_no
    GROUP BY bi.accession_number HAVING COUNT(bi.accession_no) > 2;

/* To display information on books issued to members of category other than 'F' */
SELECT b.accession_no AS 'Book Id', b.title_id AS 'Title Id', t.title_name AS 'Title Name',
    m.member_name AS 'Member Name', m.category AS 'Category' FROM books b
...
SELECT a.author_id AS 'Author Id', a.author_name AS 'Author Name' FROM authors a
