...
SELECT m.member_name AS 'Member Name', bi.member_id AS 'Member ID', t.title_name AS 'Title',
    bi.accession_no AS 'Accession Number', bi.issue_date AS 'Issue Date',
    bi.due_date AS 'Due Date', br.return_date AS 'Return Date',
    DATEDIFF(bi.issue_date,IFNULL(br.return_date, CURDATE()))/30 AS 'Number of Months' FROM book_issue bi
    ...
    WHERE DATEDIFF(bi.issue_date,IFNULL(br.return_date, CURDATE())) > 60
